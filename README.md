...
ember-cli v2.8.0 + node 4.5.0 + npm 2.15.11 + bower 1.7.9 + phantomjs 2.1.1 + watchman 3.5.0
...
1. Create new project dir and add a docker-compose.yml file similar to the following:
   ...
   ```
   ember: &defaults
     image: opinioapp/ember-cli:2.8.0
     volumes:
       - .:/myapp
...
3. Start the ember server:
   ...
   ```
   $ docker run --rm --privileged --entrypoint sysctl opinioapp/ember-cli:2.8.0 -w fs.inotify.max_user_watches=524288
   ```
